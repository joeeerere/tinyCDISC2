...
mod_dataUpload_server <- function(input, output, session) {
  ...
  observeEvent(dd$data, {
    ...
    output$radio_test <- renderUI(
      radioButtons(session$ns("select_file"),
        label = "Inspect Uploaded Data",
  ...
  observeEvent(input$pilot, {
    validate(need(
      all(input$pilot_selections %in% c("adae", "adlbc", "adsl", "adtte", "advs")),
    ...
    pilot_dat_ls <- purrr::map(input$pilot_selections, ~ switch(.x,
      adae = tidyCDISC::adae,
