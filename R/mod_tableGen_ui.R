          ...
          wellPanel(
            fluidRow(column(
              ...
              shinyUI(bootstrapPage(
                HTML('<button data-toggle = "collapse" 
                      data-target = "#demo" 
                      class = "btn btn-input filter-accordion" 
                      id = "filter-accordion">Filter Data</button>'),
                tags$div(
            ...
            wellPanel(
              fluidRow(
                column(
                  ...
                  br(),
                  div(
                    class = "col-sm-3", id = "all-column-blocks", style = "height:300px;overflow-y:scroll;overflow-x:hidden;",
                    uiOutput("all_rows")
                  ),
                  div(
                    class = "col-sm-8", id = "all-output-blocks", style = "height:300px;overflow-y:scroll;",
                  ...
                  fluidRow(
                    column(1,
                      ...
                      tags$ul(
                        ...
                        tags$li(
                          id = "nested_freq_dsc",
                          div(tippy(div("NESTED FREQ DSC"), "Subject Count at each variable level nested inside another variable's levels, sorted descending by total counts")),
                          class = "agg"
                        ),
                        tags$li(
                          id = "nested_freq_abc",
                          div(tippy(div("NESTED FREQ ABC"), "Subject Count at each variable level nested inside another variable's levels, sorted alphabetically by name")),
                          class = "agg"
                        )
                      )
                    )
