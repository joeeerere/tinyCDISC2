...
mod_popExp_server <- function(input, output, session, datafile) {
  ...
  process <- eventReactive(datafile(),
    ...
    ignoreNULL = FALSE
  ) # end of observeEvent on datafile()
  ...
  my_loaded_adams <- reactive({
    ...
    sasdata <- names(which(sapply(sasdata0, function(df) {
      return(stringr::str_detect(toupper(df), "^AD[A-Z0-9\\_]+"))
    })))
    return(sasdata)
  })
      ...
      DT::datatable(p_data(),
        ...
        options = list(
          ...
          buttons = list(list(
            extend = "excel",
            filename = paste(
              "tidyCDISC data for", input$plot_type # p_filename_base()
